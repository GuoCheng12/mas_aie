...
5. If the evidence now approaches a temporary conclusion, do not finalize directly; enter pairwise verifier mode for top1 vs top2.
6. If the next action is Macro, Microscopic, or Verifier, write a local task instruction for that agent.

Important rules:
- Do not update only the current top1. Reweight all 5 labels every round.
- If confidence is high enough for a temporary conclusion, the next action must be pairwise `Verifier`, not direct finalize.
- If action is `Verifier`, the task must explicitly distinguish top1 vs top2 and request decisive evidence rather than generic support.
...
- `decision_gate_status` should be:
  ...
  - `needs_pairwise_verifier` once the case is ready for top1-vs-top2 verifier discrimination
- If `decision_gate_status` is `needs_pairwise_verifier`, do not finalize in this stage.
