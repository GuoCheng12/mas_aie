...
def _default_prompt_repository() -> PromptRepository:
    return PromptRepository(Path(__file__).resolve().parents[2] / "prompts")
...
class MicroscopicActionDecision(BaseModel):
    status: MicroscopicActionStatus
    execution_action: Optional[AmespCapabilityName] = None
    discovery_actions: list[AmespCapabilityName] = Field(default_factory=list)
    params: dict[str, Any] = Field(default_factory=dict)
    ...
    def _validate_supported_shape(self) -> "MicroscopicActionDecision":
        if self.status == "supported":
            if self.execution_action is None:
                raise ValueError("supported microscopic action decisions must include execution_action")
            return self
        if self.execution_action is not None:
            raise ValueError("unsupported microscopic action decisions must not include execution_action")
        if self.discovery_actions:
            raise ValueError("unsupported microscopic action decisions must not include discovery_actions")
        if self.params:
            raise ValueError("unsupported microscopic action decisions must not include params")
        return self
...
class MicroscopicReasoningOutcome:
    ...
    compiled_execution_plan: Optional[MicroscopicExecutionPlan]
...
def compile_action_decision_to_execution_plan(
    decision: MicroscopicActionDecision,
    *,
    payload: dict[str, Any],
    config: AieMasConfig,
) -> Optional[MicroscopicExecutionPlan]:
    if decision.status != "supported" or decision.execution_action is None:
        return None
    action_definition = AMESP_ACTION_REGISTRY.get(decision.execution_action)
    if action_definition is None:
        raise TaggedMicroscopicProtocolError(f"Unknown execution_action '{decision.execution_action}'.")
    requested_deliverables = list(payload.get("requested_deliverables") or []) or list(
        action_definition.default_deliverables
    ) or _default_requested_deliverables_for_capability(decision.execution_action)
    unsupported_requests = list(
        dict.fromkeys(list(payload.get("unsupported_requests") or []) + list(decision.unsupported_parts))
    )
    action_card = MicroscopicActionCardDraft(
        contract_version=2,
        local_goal=(
            f"Execute the bounded microscopic action `{decision.execution_action}` for the Planner instruction: "
            f"{str(payload.get('task_instruction') or '').strip()}"
        ),
        execution_action=decision.execution_action,
        discovery_actions=list(decision.discovery_actions),
        requested_route_summary=decision.local_execution_rationale.strip()
        or _default_requested_route_summary_for_capability(decision.execution_action),
        requested_deliverables=requested_deliverables,
        unsupported_requests=unsupported_requests,
        params=dict(decision.params),
    )
    _, _, _, execution_plan = compile_action_card_to_tool_plan(
        action_card,
        action_definition=action_definition,
        ...
        budget_profile=(payload.get("budget_profile") or config.microscopic_budget_profile or "balanced"),
        available_structure_context=payload.get("available_structure_context"),
        shared_structure_context=payload.get("shared_structure_context"),
...
def _build_reasoning_response_from_action_decision(
    ...
    *,
    payload: dict[str, Any],
    compiled_plan: Optional[MicroscopicExecutionPlan],
    config: AieMasConfig,
) -> MicroscopicReasoningResponse:
    task_instruction = str(payload.get("task_instruction") or "").strip()
    current_hypothesis = str(payload.get("current_hypothesis") or "unknown")
    requested_deliverables = list(payload.get("requested_deliverables") or [])
    supported_scope = _supported_scope_descriptions(config)
    if decision.status == "supported" and decision.execution_action is not None and compiled_plan is not None:
        requested_deliverables = list(compiled_plan.requested_deliverables) or requested_deliverables
        route = compiled_plan.capability_route
        capability_name = compiled_plan.microscopic_tool_request.capability_name
        capability_limit_note = (
            "Current microscopic capability is bounded to registry-backed Amesp execution only: "
            + "; ".join(supported_scope)
            + ". It cannot make global mechanism judgments or invent new actions."
        )
        if decision.unsupported_parts:
            capability_limit_note += " Unsupported local parts were preserved as bounded omissions: " + "; ".join(
                decision.unsupported_parts
            ) + "."
        return MicroscopicReasoningResponse(
            task_understanding=(
                f"Interpret the Planner instruction as a bounded local microscopic execution task for the current "
            ...
            execution_plan=MicroscopicReasoningPlanDraft(
                ...
                requested_route_summary=compiled_plan.requested_route_summary,
                microscopic_tool_request=MicroscopicToolRequestDraft.model_validate(
                    compiled_plan.microscopic_tool_request.model_dump(mode="json")
                ),
                microscopic_tool_plan=MicroscopicToolPlanDraft.model_validate(
                    compiled_plan.microscopic_tool_plan.model_dump(mode="json")
                ),
                structure_strategy=(
                    "reuse_if_available_else_prepare_from_smiles"
                    if compiled_plan.structure_source == "existing_prepared_structure"
                    else "prepare_from_smiles"
                ),
                planning_unmet_constraints=list(compiled_plan.planning_unmet_constraints),
                unsupported_requests=list(compiled_plan.unsupported_requests),
            ),
            capability_limit_note=capability_limit_note,
            expected_outputs=list(compiled_plan.expected_outputs) or list(requested_deliverables),
            failure_policy=(
                f"If Amesp capability `{capability_name}` fails, return a local failed or partial report with available "
                "artifacts only."
            ),
        )

    unsupported_parts = list(dict.fromkeys(decision.unsupported_parts or ["unsupported microscopic intent"]))
    return MicroscopicReasoningResponse(
        task_understanding=(
            f"Interpret the Planner instruction as a local microscopic request for the current working hypothesis "
        ...
        execution_plan=MicroscopicReasoningPlanDraft(
            ...
            requested_deliverables=requested_deliverables,
            requested_route_summary="No bounded Amesp action was executed because the requested local intent is unsupported.",
            unsupported_requests=unsupported_parts,
            planning_unmet_constraints=list(unsupported_parts),
        ),
        capability_limit_note=(
            "Current microscopic capability is bounded to registry-backed Amesp execution only and cannot execute the "
            "requested local intent exactly."
        ),
        expected_outputs=list(requested_deliverables),
        failure_policy="Return a local failed report without attempting a substitute Amesp action.",
    )


def _source_round_selector_from_preference(source_round_preference: Optional[int]) -> Optional[str]:
    if source_round_preference is None:
        return None
...
def _action_decision_from_execution_plan(
    ...
    *,
    local_execution_rationale: str,
    unsupported_parts: Optional[list[str]] = None,
) -> MicroscopicActionDecision:
    request = execution_plan.microscopic_tool_request
    if request is None:
        return MicroscopicActionDecision(
            status="unsupported",
            unsupported_parts=list(unsupported_parts or ["no executable microscopic action was compiled"]),
            local_execution_rationale=local_execution_rationale,
        )
    action_definition = AMESP_ACTION_REGISTRY.get(request.capability_name)
    selection_policy = execution_plan.microscopic_tool_plan.selection_policy
    params: dict[str, Any] = {}
    if action_definition is not None:
        for param_name in action_definition.allowed_llm_params:
            value: Any = getattr(request, param_name, None)
            if value in (None, [], {}):
                if param_name == "exclude_dihedral_ids":
                    ...
                    value = selection_policy.min_relevance
                elif param_name == "include_peripheral":
                    ...
                    value = _source_round_selector_from_preference(selection_policy.source_round_preference)
            if value in (None, [], {}):
                continue
            params[param_name] = value
    discovery_actions = [
        call.request.capability_name
        for call in execution_plan.microscopic_tool_plan.calls
        if call.call_kind == "discovery"
    ]
    return MicroscopicActionDecision(
        status="supported",
        execution_action=request.capability_name,
        discovery_actions=discovery_actions,
        params=params,
